...
def metric_prf(ys, y_hat, theta, gt_theta=1e-4,xs=None):
    p = []
    r = []
    f = []
    num_sample = len(xs)
...
def RAN(root, seq, mode):
    ...
    p, r, f = metric_prf(ys, y_hat, th, xs=xs)
    p_avg = np.mean(p)
    r_avg = np.mean(r)
    f_avg = np.mean(f)
    print(p_avg, r_avg, f_avg)

def SIFT(root, seq, mode):
    KRT_img = load_data(root, seq, mode, 'KRT_img.pkl')
    K = KRT_img['K']
    R = KRT_img['R']
    T = KRT_img['t']
    img_path = KRT_img['img_path']
    xs = load_data(root, seq, mode, 'xs_S.pkl')['xs'] # unnormalize [initail match] shape:pairnum*1*kp*4
    ys = []# to save sym_epi_distance
    for ii in range(0, len(K), 2): #步长为2 ii=0 2 4 6 8 10 12...
        jj = ii+1
        # normalize_keypoints
